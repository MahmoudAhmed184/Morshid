...
When review is needed, the private governance boundary calls
`ReviewCaseIntake.openAutomatic` once for every stable reason, using a
deterministic per-message/per-reason event key and the caller-owned database
transaction. The intake aggregates those triggers onto the database's one
...
`server/src/modules/tutoring/response-governance/automatic-safety.fixtures.ts` is the CI
...
clean-only on the ordinary tutoring path until those slices activate them.
...
When trusted retrieval is unavailable for a covered request, the tutor model is
not called. The assistant message is finalized directly with the fixed safe
...
Detection runs before model generation. A match skips the provider, persists the
...
before model generation. Proposed tutor response content is checked before any content or
...
keeps injected document text and unsafe tutor response content out of messages,
