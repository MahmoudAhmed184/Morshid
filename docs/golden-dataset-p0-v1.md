...
> [!IMPORTANT]
> This dataset uses the committed source PDFs under
> `fixtures/course-materials/` (source IDs `p0-npt-part-01` to `p0-npt-part-05`, `SCN-003_Fixture_A`, and `SCN-003_Fixture_B`). Their
> attribution, license, and transformation notes are recorded in
> `fixtures/course-materials/ATTRIBUTION.md`.

## Purpose

This document defines the locked golden evaluation dataset for Morshid's
source and evaluation baseline. It provides human-readable dataset definitions
backing automated tests, evaluation runs, and demo verification.

The dataset aligns with the grouped Python part PDFs.
It verifies that Morshid can:
...
These source IDs are stable for this dataset version. Licensing and source
transformation details are in `fixtures/course-materials/ATTRIBUTION.md`.
...
These items cover unsupported requests, prompt injection, and course isolation
behavior required by Story 1.9 and `docs/project-delivery-plan.md`. Expected
source coverage is intentionally "none". Type-level expectations from the
type-level expectations section apply. Authorization items use seeded accounts
and courses from `docs/demo-scenario-mapping.md`.
...
This table maps demo scenarios from `docs/demo-scenario-mapping.md` to backing
golden dataset items. Demo scenario IDs stay stable across scenario mapping,
fixtures, expected outputs, and evaluation runs.
...
| SCN-003 (conflicting-source) | `fixtures/course-materials/SCN-003_Fixture_A.pdf`, `SCN-003_Fixture_B.pdf` | Dedicated conflicting-source PDF fixtures back SCN-003 without mutating the 65 locked item IDs. |
...
- Keep existing item IDs stable across evaluation runs.
...
- If a prompt, expected classification, or expected behavior changes after
  fixtures are created, record that as a dataset update following fixture update
  conventions.
- Companion conflicting-source fixtures exist in `fixtures/course-materials/`
  (`SCN-003_Fixture_A.pdf` and `SCN-003_Fixture_B.pdf`) to support SCN-003 conflict
  detection testing.
- Version strings follow `p0-v<major>-<YYYY-MM-DD>-<suffix>`, where the date is
  the lock date and the suffix names the source set (`part-sources` = the grouped
  part PDFs committed under `fixtures/course-materials/`).
