...
- Uses Node native crypto's `argon2Sync('argon2id')`.
- Parameters:
...
- Constant-time fallback: If a user is not found during sign-in, the hasher runs verification against `DUMMY_PASSWORD_HASH` with `timingSafeEqual` to prevent timing-based user enumeration.
...
- Payload structure:
  ...
  ```
- Instant invalidation: During authentication, `IdentityGuard` verifies that `payload.pwd === user.passwordChangedAt.toISOString()`. When a user changes or resets their password, existing JWT access tokens fail this check immediately without waiting for expiration.
