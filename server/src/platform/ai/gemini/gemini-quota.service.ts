...
/**
 * Which family of budgets a guard instance meters into.
 *
 * There is no default. Gemini's free-tier limits are per Google project, so two
 * workloads on separate projects must never share a bucket and two workloads on
 * one project must share it exactly — neither is something this service can
 * infer, and a silently inherited namespace would collapse an embedding ingest
 * onto the interactive chat budget (or vice versa) with no visible symptom
 * until one starves the other.
 *
 * `keySalt` is domain separation, deliberately not a secret: its only job is to
 * make the stored key a digest that exists nowhere else, so a Redis key can
 * never be compared against a digest computed elsewhere from the same identity.
 * Bumping its version suffix retires every existing bucket in that namespace,
 * which is exactly what a change to the hash field layout requires.
 */
export interface GeminiQuotaNamespace {
 ...
 *
 * The `project` variant identifies the Google project by an opaque deployment
 * label instead. It is the right shape when several replicas must share one
 * budget and a credential rotation must not mint a fresh day or month window —
 * the label survives rotation, the credential does not. The label is operator
 * vocabulary, never the real Google project name, and it is hashed on the same
 * terms as a credential so nothing about the deployment reaches Redis either
 * way. Choosing between the two is choosing what a rotation means, so the
 * variants are separate rather than one nullable field.
 */
export type GeminiQuotaDeployment =
  { readonly credential: string } | { readonly project: string }
...
export class GeminiQuotaService {
  ...
  constructor(
    ...
    namespace: GeminiQuotaNamespace,
  ) {
    this.key = deriveQuotaKey(deployment, namespace)
  }
...
function deriveQuotaKey(
  deployment: GeminiQuotaDeployment | string,
  namespace: GeminiQuotaNamespace,
): string {
  return `${namespace.keyPrefix}${createHmac('sha256', namespace.keySalt)
    .update(describeIdentity(deployment))
    .digest('hex')
    .slice(0, QUOTA_KEY_DIGEST_HEX_LENGTH)}`
}

// The `kind:value` prefixes are what keep the identity variants from colliding:
// a project label that happens to equal a credential must not land on the same
// bucket. They are part of the stored digest, so none of them may be renamed
// without retiring the namespace's salt.
function describeIdentity(deployment: GeminiQuotaDeployment | string): string {
  ...
  // and should be replaced by `{ credential }` or `{ project }`.
  if (typeof deployment === 'string') {
    return `model:${deployment}`
  }

  return 'credential' in deployment
