...
// One retry. A completion is a single interactive turn behind a hard deadline,
// so the budget exists to absorb one transient blip, not to ride out an
 ...
 *
 * Rejecting with the fixed cancellation code keeps an aborted wait
 * indistinguishable from any other cancelled step, and the timer and listener
 * are released on every exit so a long delay cannot outlive the request.
 */
