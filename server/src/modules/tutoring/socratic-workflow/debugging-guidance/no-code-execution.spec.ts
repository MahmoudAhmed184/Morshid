...
const tutoringSourceRoot = join(serverSourceRoot, 'modules', 'tutoring')
const debuggingGuidanceSourceRoot = join(
  tutoringSourceRoot,
  'socratic-workflow',
  'debugging-guidance',
)
...
describe('Student-code no-execution architecture', () => {
  ...
  const debuggingGuidanceSourceFiles = collectProductionTypeScriptFiles(
    debuggingGuidanceSourceRoot,
  )
  ...
  it('contains no execution, package-install, or dynamic-import path in Tutor production code', () => {
    const matches = debuggingGuidanceSourceFiles.flatMap((filePath) => {
      const source = readFileSync(filePath, 'utf8')
