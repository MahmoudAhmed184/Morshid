...
export interface DebuggingGuidanceStrategyInput {
...
const KNOWN_LANGUAGE_KEYWORDS = new Set([
  'and',
  'as',
  'assert',
  'async',
...
function diagnoseMissingFunctionColon(
  ...
  return {
    diagnosis: {
      ...
      conceptExplanation:
        'Block headers require the delimiter expected by the language before their body.',
      nextInspectionStep:
...
function diagnoseIndexAtLength(
  ...
  return {
    diagnosis: {
      ...
      conceptExplanation:
        'Sequence indexes have a bounded valid range, so the collection length is not itself a valid final index.',
      nextInspectionStep:
  ...
  for (let index = 0; index < structuralLines.length - 1; index += 1) {
    ...
    return {
      diagnosis: {
        ...
        conceptExplanation:
          'A loop body must be structurally attached to its loop header, and indentation is one common way languages express that block relationship.',
        nextInspectionStep:
...
function diagnoseFilePathEscapes(code: string): DiagnosisMatch | null {
  ...
  return {
    diagnosis: {
      ...
      conceptExplanation:
        'String literals can interpret escape sequences before a path-handling API receives the value.',
      nextInspectionStep:
...
function diagnoseStringConcatenation(code: string): DiagnosisMatch | null {
  ...
  return {
    diagnosis: {
      ...
      conceptExplanation:
        'String concatenation requires operands with compatible types.',
      nextInspectionStep:
...
function diagnoseNameLookup(structuralCode: string): DiagnosisMatch | null {
  ...
  const unresolved = identifiers.find(
    (identifier) =>
      ...
      !KNOWN_LANGUAGE_IDENTIFIERS.has(identifier) &&
      !KNOWN_LANGUAGE_KEYWORDS.has(identifier),
  )
  ...
  return {
    diagnosis: {
      ...
      conceptExplanation: likelyMismatch
        ? `Name lookup searches the active scope, where \`${nearest.candidate}\` exists but \`${unresolved}\` does not.`
        : `Expression identifiers are resolved through visible scopes, and \`${unresolved}\` is not visibly defined in this function.`,
...
function retrievalInput(
  suspectedCategory: DebuggingGuidanceRetrievalQueryInput['suspectedCategory'],
  locationHint: DebuggingGuidanceRetrievalQueryInput['locationHint'],
  signal: DebuggingGuidanceRetrievalQueryInput['diagnosticSignals'][number],
): DebuggingGuidanceRetrievalQueryInput {
  return {
    suspectedCategory,
    locationHint,
...
function parseDiagnosisDraft(value: DebuggingGuidanceDraft) {
  const parsed: DebuggingGuidance = debuggingGuidanceSchema.parse({
    ...value,
    citations: [],
  })
