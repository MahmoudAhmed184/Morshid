import { countNormalizedCodeLines } from './debugging-guidance.boundary'
import type {
  DebuggingGuidanceResponseGovernanceResult,
  DebuggingGuidance,
} from './debugging-guidance.contract'

export const DEBUGGING_GUIDANCE_OUTPUT_BLOCKED =
  'DEBUGGING_GUIDANCE_RESPONSE_GOVERNANCE_BLOCKED'
...
interface DebuggingGuidanceOutputGuardInput {
...
export function validateDebuggingGuidanceOutput(
  input: DebuggingGuidanceOutputGuardInput,
): DebuggingGuidanceResponseGovernanceResult {
  ...
  if (
    ...
    extractDebuggingGuidanceCitationIndexes(
      input.content,
      conceptSection,
...
export function debuggingGuidanceOutputErrorCode(
  result: Exclude<
    DebuggingGuidanceResponseGovernanceResult,
    'ALLOWED_DIAGNOSIS'
  >,
): string {
  return `${DEBUGGING_GUIDANCE_OUTPUT_BLOCKED}_${result}`
}

export function buildSafeDebuggingGuidanceFallback(
  diagnosis: Omit<DebuggingGuidance, 'citations'>,
): string {
  return [
...
function extractDebuggingGuidanceCitationIndexes(
  content: string,
...
export function readDebuggingGuidanceCitationIndexes(
  content: string,
  authorizedCitationCount: number,
): readonly number[] | null {
