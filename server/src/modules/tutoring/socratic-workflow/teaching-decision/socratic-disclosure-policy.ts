...
export interface SocraticDisclosureContract {
  ...
  readonly boundedConceptualExplanationAllowed: boolean
...
export function buildSocraticDisclosureContract(input: {
  ...
  const permitsCompleteSolution =
    input.revealPolicy === RevealPolicy.COMPLETE_SOLUTION_ALLOWED
  const boundedConceptualExplanationAllowed =
    input.requestKind === MessageRequestKind.CONCEPTUAL &&
    input.revealPolicy === RevealPolicy.PARTIAL_RESULT_ALLOWED &&
  ...
  return Object.freeze({
    ...
    boundedConceptualExplanationAllowed,
    ...
    intermediateResultAllowed:
      boundedConceptualExplanationAllowed ||
      (guidanceLevel >= 3 && permitsPartialResult),
    ...
    requiredStudentWork: requiredStudentWork(
      guidanceLevel,
      boundedConceptualExplanationAllowed,
    ),
  })
...
function requiredStudentWork(
  guidanceLevel: number,
  boundedConceptualExplanationAllowed: boolean,
): string {
  if (boundedConceptualExplanationAllowed) {
    return 'State one concise grounded core explanation, then ask the student for one meaningful comparison, prediction, application, or reflection.'
  }
