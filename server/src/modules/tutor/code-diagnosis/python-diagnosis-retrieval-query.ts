...
export const PYTHON_DIAGNOSIS_LOCATION_HINTS = [
  'FUNCTION_HEADER',
  'FUNCTION_BODY',
  'RETURN_EXPRESSION',
  'LOOP_HEADER',
...
export const PYTHON_DIAGNOSTIC_SIGNALS = [
  'MISSING_BLOCK_COLON',
...
export const pythonDiagnosisRetrievalQueryInputSchema = z
  .object({
    language: z.literal('python'),
    suspectedCategory: z.enum(PYTHON_DIAGNOSIS_CATEGORIES),
    locationHint: z.enum(PYTHON_DIAGNOSIS_LOCATION_HINTS),
    diagnosticSignals: z.array(z.enum(PYTHON_DIAGNOSTIC_SIGNALS)).min(1).max(5),
...
export type PythonDiagnosisRetrievalQueryInput = z.infer<
  typeof pythonDiagnosisRetrievalQueryInputSchema
>
...
const CATEGORY_QUERY_TEXT = {
  ...
  PythonDiagnosisRetrievalQueryInput['suspectedCategory'],
  string
>
...
const LOCATION_QUERY_TEXT = {
  ...
  PythonDiagnosisRetrievalQueryInput['locationHint'],
  string
>
...
const SIGNAL_QUERY_TEXT = {
  ...
  PythonDiagnosisRetrievalQueryInput['diagnosticSignals'][number],
  string
>

export function buildPythonDiagnosisRetrievalQuery(value: unknown): string {
  const input = pythonDiagnosisRetrievalQueryInputSchema.parse(value)
  ...
  return `Python ${problem} near ${location}; study ${concept}. Diagnostic signals: ${signals}.${identifiers}`
}
