...
import {
  ...
  pythonDiagnosisRetrievalQueryInputSchema,
...
describe('Python diagnosis retrieval-query contract', () => {
  it('builds a stable problem-based gd-p0-v1-058 query', () => {
    expect(buildPythonDiagnosisRetrievalQuery(nameLookupInput())).toBe(
  ...
  it.each([
    ...
    (suspectedCategory, signal, locationHint) => {
      const query = buildPythonDiagnosisRetrievalQuery({
        language: 'python',
        suspectedCategory,
        locationHint,
  ...
  it('covers the same categories as the representative fixture contract', () => {
    const schemaCategories = new Set(PYTHON_DIAGNOSIS_CATEGORIES)
  ...
  it.each([
    ...
    expect(() =>
      pythonDiagnosisRetrievalQueryInputSchema.parse(input),
    ).toThrow()
  })
  ...
  it('does not copy injection fixture content into a derived query', () => {
    ...
    const query = buildPythonDiagnosisRetrievalQuery(nameLookupInput())
