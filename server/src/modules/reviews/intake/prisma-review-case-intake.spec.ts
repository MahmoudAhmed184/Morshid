...
import { PrismaReviewCaseIntake } from './prisma-review-case-intake'
import type { AutomaticReviewIntakeInput } from '../interface/review-case-intake'
...
describe('PrismaReviewCaseIntake', () => {
  ...
  it('rejects excess evidence at the Reviews intake boundary', async () => {
    await expect(
      intake.openAutomatic(
        {
          messageId,
          triggers: [
            {
              ...
              sourceEventKey: 'excess-evidence',
            },
          ],
          evidence: {
            summary: 'Too many sources',
            sources: Array.from({ length: 21 }, (_, index) => ({
              excerpt: `Source ${String(index + 1)}`,
            })),
          },
        },
        transaction,
      ),
    ).rejects.toThrow('evidence.sources must contain at most 20 items')
    expect(createAutomaticInTransaction).not.toHaveBeenCalled()
  })
