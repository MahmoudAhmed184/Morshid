import {
  ...
  readUpstreamFailure,
  waitForRetry,
} from './upstream-retry-policy'

// The shared helper has no error vocabulary of its own; every caller supplies
// one. This sentinel stands in for whichever error the caller would raise.
class CancelledSentinelError extends Error {
  constructor() {
    super('cancelled sentinel')
    this.name = 'CancelledSentinelError'
  }
}

const createCancellationError = () => new CancelledSentinelError()
...
describe('waitForRetry', () => {
  ...
  it('resolves after the delay and releases its timer and listener', async () => {
    ...
    const pending = waitForRetry(40, controller.signal, createCancellationError)
  ...
  it('rejects immediately when the signal is already aborted', async () => {
    ...
    await expect(
      waitForRetry(30_000, controller.signal, createCancellationError),
    ).rejects.toBeInstanceOf(CancelledSentinelError)
    expect(jest.getTimerCount()).toBe(0)
  })

  it("rejects with the caller's own cancellation error when aborted mid-wait", async () => {
    const controller = new AbortController()
    ...
    await expect(pending).rejects.toBeInstanceOf(CancelledSentinelError)
  ...
  it('ignores an abort that arrives after the delay already elapsed', async () => {
    const controller = new AbortController()
    const pending = waitForRetry(10, controller.signal, createCancellationError)
