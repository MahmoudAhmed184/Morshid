import { authSessionSchema } from '@/features/auth/session/session.schema'
...
import type { ApiFetchOptions } from '@/lib/http/http'

export { ApiError, isApiError } from '@/lib/http/http'
export type { ApiFetchOptions } from '@/lib/http/http'
...
function refreshSession(
  ...
  const refreshKey = String(sessionVersion)
  ...
  const refreshPromise = fetchImpl(
    ...
    .then(async (response) => {
      ...
      const wasApplied = useAuthStore
        .getState()
        .setRefreshedSession(session, sessionVersion)
...
export async function restoreAuthSession(
  ...
  const { apiBaseUrl = clientEnv.VITE_API_BASE_URL, fetchImpl = fetch } =
    options
  let previousSessionVersion: number | null = null
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (sessionVersion === previousSessionVersion) {
      return null
    }

    try {
      return await refreshSession(sessionVersion, fetchImpl, apiBaseUrl)
    } catch (error) {
      if (error instanceof AuthSessionChangedError) {
        previousSessionVersion = sessionVersion
        continue
      }
...
async function apiFetchWithAuthRetry(
  ...
  try {
    await refreshSession(sessionVersion, fetchImpl, apiBaseUrl)
  } catch (refreshError) {
