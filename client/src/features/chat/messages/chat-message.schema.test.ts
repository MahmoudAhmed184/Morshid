...
describe('Student chat contract schemas', () => {
  ...
  it('validates strict grounded-turn citations and availability invariants', () => {
    expect(chatTurnResponseSchema.parse(chatTurnResponseFixture)).toEqual(
      chatTurnResponseFixture,
    )

    const citation = chatTurnResponseFixture.assistantMessage.citations[0]
  ...
  it('rejects cross-shaped and non-terminal complete grounded turns', () => {
    for (const response of [
      ...
      expect(chatTurnResponseSchema.safeParse(response).success).toBe(false)
    }
  })
  ...
  it('accepts only trimmed tutoring content within 4,000 Unicode code points', () => {
    ...
    for (const input of [
      ...
      expect(sendChatMessageRequestSchema.safeParse(input).success).toBe(false)
    }
  })
  ...
  it('validates optional AI Tutor course and session search state', () => {
    ...
    expect(
      chatSearchSchema.parse({
        courseId: 'course-id',
        sessionId: 'session-id',
      }),
    ).toEqual({ courseId: 'course-id', sessionId: 'session-id' })
    expect(() => chatSearchSchema.parse({ courseId: '' })).toThrow()
    expect(() => chatSearchSchema.parse({ sessionId: '' })).toThrow()
  })
})
