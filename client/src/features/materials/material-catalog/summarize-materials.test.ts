...
import { summarizeMaterials } from './summarize-materials'

const courseId = 'f5bb713c-09b7-42d3-acf3-02f39a902e5a'

function material(id: string, status: Material['status']): Material {
  return {
    id,
    courseId,
...
describe('summarizeMaterials', () => {
  it('returns zero counts for an empty material list', () => {
    expect(summarizeMaterials([])).toEqual({
      total: 0,
      processing: 0,
      ready: 0,
      attention: 0,
  ...
  it('buckets every material status into the shared dashboard totals', () => {
    ...
    expect(summarizeMaterials(materials)).toEqual({
      total: 4,
      processing: 1,
      ready: 1,
      attention: 2,
