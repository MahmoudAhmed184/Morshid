...
} from './material-ingestion.queries'
...
function material(status: Material['status']): Material {
  return {
...
function listPollingIntervalFor(
  ...
  const options = materialsQueryOptions(scope)
  ...
  return refetchInterval({
    state: {
      data: {
        pages: [{ materials: [material(status)] }],
...
describe('Instructor material query options', () => {
  it('partitions list and configuration keys by Instructor and course', () => {
    expect(materialKeys.list(scope)).toEqual([
      'instructor',
      scope.instructorId,
      'materials',
      'list',
    ...
    expect(materialKeys.uploadConfiguration(scope.instructorId)).toEqual([
      'instructor',
      scope.instructorId,
      'materials',
    ...
    ).not.toEqual(materialKeys.list(scope))
  })
