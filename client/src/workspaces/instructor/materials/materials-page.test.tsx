...
const useMaterialUploadConfigurationMock = vi.mocked(
  useMaterialUploadConfiguration,
)
...
describe('MaterialsPage', () => {
  beforeEach(() => {
    ...
    useMaterialUploadConfigurationMock.mockReturnValue(
      queryResult({
        maxUploadBytes: 1_024,
        acceptedMimeType: 'application/pdf',
        acceptedFileExtension: '.pdf',
      }) as unknown as ReturnType<typeof useMaterialUploadConfiguration>,
    )
    useUploadCourseMaterialMock.mockReturnValue({
      mutate: vi.fn(),
      mutateAsync: uploadCourseMaterial,
      isPending: false,
      error: null,
  ...
  it('shows the assigned-course empty state', () => {
    useCourseMembershipMock.mockReturnValue(
      queryResult([]) as unknown as ReturnType<typeof useCourseMembership>,
    )
  ...
  it('renders the redesigned summary, repository, and material metadata', () => {
    ...
    expect(useCourseMaterialsMock).toHaveBeenCalledWith(course.id)
  ...
  it('switches material data, summaries, and upload target with the selected course', async () => {
    ...
    uploadCourseMaterial.mockResolvedValue({ material: secondCourseMaterial })
    const user = userEvent.setup()

    renderMaterialsPage()

    expect(useCourseMaterialsMock).toHaveBeenLastCalledWith(course.id)
    ...
    expect(useCourseMaterialsMock).toHaveBeenLastCalledWith(secondCourse.id)
    expect(screen.getByLabelText('Select assigned course')).toHaveTextContent(
    ...
    await waitFor(() =>
      expect(uploadCourseMaterial).toHaveBeenCalledWith({
        courseId: secondCourse.id,
        title: 'Graph Theory',
        file,
      }),
  ...
  it('shows inline upload validation errors', async () => {
    ...
    expect(uploadCourseMaterial).not.toHaveBeenCalled()
  })
  ...
  it('replaces the form with upload progress while submit is pending', async () => {
    ...
    uploadCourseMaterial.mockReturnValue(uploadPromise)
    const user = userEvent.setup()
  ...
  it('renders a typed server error and succeeds after the form is reset', async () => {
    ...
    expect(uploadCourseMaterial).toHaveBeenCalledTimes(2)
  })

  it('renders a safe generic message for an unexpected upload failure', async () => {
    uploadCourseMaterial.mockRejectedValueOnce(
  ...
  it('uploads a valid PDF and resets the form after success', async () => {
    uploadCourseMaterial.mockResolvedValue({ material })
    const user = userEvent.setup()
